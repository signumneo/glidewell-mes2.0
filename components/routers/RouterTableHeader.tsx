...
export function RouterTableHeader({
  ...
  const getSortIcon = (columnKey: string, isHovered: boolean) => {
    const isActive = sortField === columnKey;
    
    if (!isActive && !isHovered) return null;
    
    if (!isActive) return <ArrowUpDown className="w-4 h-4 opacity-40" />;
      ...
      {columns.map((column) => {
        ...
        return (
          <div
            ...
            className="relative flex items-center justify-center px-4 py-3 font-semibold text-sm text-gray-700 dark:text-gray-300"
          >
            <button
            ...
            >
              {column.label}
              {column.sortable && getSortIcon(column.key, isHovered)}
            </button>
